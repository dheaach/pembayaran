...
Class Mongo_db{
    ...
    */
    ...
    public function storeUpload($collection = "", $filename, $insert = array(), $path)
    {
        if (empty($collection))
        {
            show_error("No Mongo collection selected to insert into", 500);
        ...
        $bucket = new MongoDB\GridFS\Bucket($manager, 'fhs_image');
    
        try
        {
            $result = $bucket->uploadFromStream($filename, $path);
            return $result;
        }
        // Check if the write concern could not be fulfilled
        catch (MongoCursorException $e)
        {
            show_error("MongoDB store upload failed: {$e->getMessage()}", 500);
        }
    }
