...
var Dashboard = (function() {
  ...
  var dailySales = function() {
    ...
    var chartData = {
      ...
      datasets: [
        ...
        //     data: [10, 20, 30, 40, 50, 60, 70, 80, 90, 110, 120, 130, 140, 150, 160, 17]
        // }
      ]
    };
  ...
  var daterangepickerInit = function() {
    ...
    function cb(start, end, label) {
      ...
      picker
        .find("#m-subheader__daterange-start-hidden")
        .val(start.format("Y-M-D"));
      picker
        .find("#m-subheader__daterange-end-hidden")
  ...
  var dashboardInit = function() {
    ...
    $("#m_dashboard_daterangepicker").on("apply.daterangepicker", function() {
      var startDate = $("#m-subheader__daterange-start-hidden").val(),
        endDate = $("#m-subheader__daterange-end-hidden").val();

        $.ajax({
            url: site_url+"dashboard/getdatadashboard",
            dataType: 'json',
            type: 'POST',
            data : {
                startdate: startDate,
            ...
            success: function(data, textStatus, XMLHttpRequest) {
                if( data.success ) {
                    ...
                    $('.txt-transaksi-penjualan').html(data.transaksi_penjualan_format);
                    $('.txt-total-omset').html(data.total_omset_format);
                    $('.txt-avg-penjualan').html(data.avg_penjualan_format);
                    var dt = [];
                    dt.push(data.data1);
                    changeData(chart, data.label1, dt);
                }
                else {
                    swal("Dashboard", "Data transaksi tidak ditemukan", "error");
                }
            }
        });
    });

    $('#m_dashboard_daterangepicker').trigger('apply.daterangepicker');

    function changeData(crt, label, data) {
        console.log(["UPDATE", crt.data.datasets]);
        crt.data.labels = label;
        var dt = 0;
        crt.data.datasets.forEach(dataset => {
            dataset.data = data[dt].map(function(e) {
                return e;
            });
            dt++;
        });
